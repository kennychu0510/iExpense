...
describe('Scenario 6: Payers with different amount', () => {
  ...
  test('john pays and receives $0', () => {
    const name = 'john';
    expect(calculateExpenseSplitSummary(people).find((person) => person.name === name)?.payActions).toBeEmpty();
    expect(calculateExpenseSplitSummary(people).find((person) => person.name === name)?.receiveActions).toBeEmpty();
    expect(
      calculateExpenseSplitSummary(people)
